  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      this.pokemonService.getPokemon(id).subscribe((pokemon) => {
        ...
        this.hp_percentage = Math.round((this.hp_stat / 160) * 100);
        this.attack_stat = pokemon.stats[1].base_stat;
        this.attack_percentage = Math.round((this.attack_stat / 160) * 100);
        this.defense_stat = pokemon.stats[2].base_stat;
        this.defense_percentage = Math.round((this.defense_stat / 160) * 100);
        this.special_attack_stat = pokemon.stats[3].base_stat;
        this.special_attack_percentage = Math.round((this.special_attack_stat / 160) * 100);
        this.special_defense_stat = pokemon.stats[4].base_stat;
        this.special_defense_percentage = Math.round((this.special_defense_stat / 160) * 100);
        this.speed_stat = pokemon.stats[5].base_stat;
        this.speed_percentage = Math.round((this.speed_stat / 160) * 100);
      });
    }
  }
